...
export default function AppHeader({ variant = 'global', contextBar, subBar }: AppHeaderProps) {
  ...
  const navLinkClass = (path: string) => {
    const base = 'px-3 sm:px-4 py-2 rounded-xl font-medium transition-all duration-200 flex items-center font-manrope';
  ...
  const desktopNav = (
    <div className="hidden md:block border-t border-manah-gold/10">
      <div className="flex items-center justify-center gap-2 px-4 py-2">
        <Link to="/inicio" className={navLinkClass('/inicio')} data-tutorial="nav-inicio">
          <svg className="w-5 h-5 inline-block mr-1" fill="currentColor" viewBox="0 0 24 24"><path d="M10 20v-6h4v6h5v-8h3L12 3 2 12h3v8z"/></svg>
          Inicio
        </Link>
        {!isAdmin && (
          <>
        ...
        {isAdmin && (
          <Link to="/admin/config" className={navLinkClass('/admin/config')}>
            <svg className="w-5 h-5 inline-block mr-1" fill="currentColor" viewBox="0 0 24 24"><path d="M19.14 12.94c.04-.3.06-.61.06-.94 0-.32-.02-.64-.07-.94l2.03-1.58c.18-.14.23-.41.12-.61l-1.92-3.32c-.12-.22-.37-.29-.59-.22l-2.39.96c-.5-.38-1.03-.7-1.62-.94L14.4 2.81c-.04-.24-.24-.41-.48-.41h-3.84c-.24 0-.43.17-.47.41l-.36 2.54c-.59.24-1.13.57-1.62.94l-2.39-.96c-.22-.08-.47 0-.59.22L2.74 8.87c-.12.21-.08.47.12.61l2.03 1.58c-.05.3-.09.63-.09.94s.02.64.07.94l-2.03 1.58c-.18.14-.23.41-.12.61l1.92 3.32c.12.22.37.29.59.22l2.39-.96c.5.38 1.03.7 1.62.94l.36 2.54c.05.24.24.41.48.41h3.84c.24 0 .44-.17.47-.41l.36-2.54c.59-.24 1.13-.56 1.62-.94l2.39.96c.22.08.47 0 .59-.22l1.92-3.32c.12-.22.07-.47-.12-.61l-2.01-1.58zM12 15.6c-1.98 0-3.6-1.62-3.6-3.6s1.62-3.6 3.6-3.6 3.6 1.62 3.6 3.6-1.62 3.6-3.6 3.6z"/></svg>
            Configuración
          </Link>
        )}
      </div>
      ...
      <nav className={`fixed top-0 left-0 right-0 z-50 font-manrope ${isAdmin ? 'bg-gradient-to-r from-orange-900 to-red-900' : 'bg-manah-bg'}`}>
        <div className="max-w-7xl mx-auto">
          ...
          {variant === 'reader' && contextBar && (
            <div className="px-3 sm:px-4 py-3 sm:py-4 border-b border-manah-gold/20">
              <div className="max-w-6xl mx-auto flex items-center justify-between gap-4">
                <div className="flex-shrink-0">{contextBar.left}</div>
                <div className="flex-1 text-center min-w-0">{contextBar.center}</div>
                <div className="flex-shrink-0">{contextBar.right}</div>
              </div>
            </div>
          )}

          {/* subBar (both variants) */}
          {subBar && (
            <div className="border-t border-manah-gold/10 px-3 sm:px-4 py-2">
              {subBar}
            </div>
          )}
